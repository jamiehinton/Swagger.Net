namespace Swagger.Net
{
    ...
    public static class SwaggerGen
    {
        ...
        public static ResourceListing CreateResourceListing( HttpActionContext actionContext,
                                                             bool includeResourcePath = true )
        {
        ...
        private static string GenerateBasePathOrUseOverride( HttpControllerContext controllerContext )
        {
            if( HttpRuntime.AppDomainAppVirtualPath == null )
                throw new NullReferenceException( "HttpRuntime is null, Swagger.Net will only work in an Http context." );

            var uri = controllerContext.Request.RequestUri;
            var forcedSwaggerBaseUri = ConfigurationManager.AppSettings[ "SwaggerBaseUri" ];
            return string.IsNullOrWhiteSpace( forcedSwaggerBaseUri )
                       ? string.Format( "{0}{1}", uri.GetLeftPart( UriPartial.Authority ), HttpRuntime.AppDomainAppVirtualPath.TrimEnd( '/' ) )
                       : forcedSwaggerBaseUri;
        }
        ...
        public static ResourceApiOperationParameter CreateResourceApiOperationParameter( ApiDescription api,
                                                                                             ...
                                                                                             docProvider )
        {
            ...
            ResourceApiOperationParameter parameter = new ResourceApiOperationParameter
                {
                    paramType =
                        ( paramType == "query" && api.RelativePath.IndexOf( "{" + param.Name + "}" ) > -1 )
                            ? PATH
                            : paramType,
                    name = param.Name,
